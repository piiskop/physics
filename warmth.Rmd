...
Carnot' soojusmasina kasutegur:

```{=tex}
\begin{align}
\frac{Q_h}{W} := \frac{T_h}{T_h - T_c}
(\#eq:coefficient-of-performance-in-carnot-heat-engine)
\end{align}
```
...
```{r label = "iso-processes", fig.cap = "Isoprocesses", echo = FALSE}
include_svg("heat/iso-processes.svg")
```
...
\begin{align}
V_m :&= \frac{V}{n} \\
&= \frac{`r R` \times \mathrm{J} \times T}{\mathrm{K} \times \mathrm{mol} \times 1 \times 10^5 \times Pa} &
...
```{=tex}
\begin{equation}
W = n \times `r R` \times \mathrm{\frac{J}{K \times mol}} \times \Delta(T) 
...
```{=tex}
\begin{equation}
W = \frac{V \times `r R` \times \mathrm{J} \times \Delta(T)}{V_m \times \mathrm{K \times mol}} 
...
```{=tex}
\begin{align}
W :&= \frac{V_0 \times `r R` \times \mathrm{J} \times (\frac{T_0 \times V}{V_0} - T_0)}{V_m \times \mathrm{K} \times \mathrm{mol}} \\
&= \frac{V_0 \times `r R` \times \mathrm{J} \times T_0 \times (\frac{V}{V_0} - 1)}{V_m \times \mathrm{K} \times \mathrm{mol}} \\
&= \frac{V_0 \times `r R` \times \mathrm{J} \times T_0 \times (\frac{V - V_0}{V_0})}{V_m \times \mathrm{K} \times \mathrm{mol}} \\
&= \frac{`r R` \times \mathrm{J} \times T_0 \times (V - V_0)}{V_m \times \mathrm{K} \times \mathrm{mol}} \\
&= \frac{`r R` \times \mathrm{J} \times T_0 \times (V - V_0)}{\frac{`r R` \times \mathrm{J} \times T_0}{\mathrm{K} \times \mathrm{mol} \times 1 \times 10^5 \times Pa} \times \mathrm{K} \times \mathrm{mol}} \\
&= \frac{(V - V_0) \times \mathrm{K} \times \mathrm{mol} \times 1 \times 10^5 \times Pa}{\mathrm{K} \times \mathrm{mol}} \\
...
&= \frac{V_0 \times C_V \times (T_0 - T)}{\frac{`r R` \times \mathrm{J} \times T_0}{\mathrm{K} \times \mathrm{mol} \times p}} \\
&= \frac{V_0 \times C_V \times (T_0 - T) \times \mathrm{K} \times \mathrm{mol} \times p}{`r R` \times \mathrm{J} \times T_0} \\
...
```{=tex}
\begin{align}
C_V = \frac{3 \times `r R` \times \mathrm{J}}{2 \times \mathrm{K} \times \mathrm{mol}}
...
```{=tex}
\begin{align}
W :&= \frac{V_0 \times \frac{3 \times `r R` \times \mathrm{J}}{2 \times \mathrm{K} \times \mathrm{mol}} \times (T_0 - T) \times \mathrm{K} \times \mathrm{mol} \times p}{`r R` \times J \times T_0} \\
...
Omavahel on konstantse ruumala ja rõhu molaarsed erisoojused seotud molaarse gaasi konstandi kaudu:

```{=tex}
\begin{align}
C_p := C_V + 8.3144621(75) \times \mathrm{\frac{J}{mol \times K}} = \frac{8.3144621(75) \times \mathrm{J} \times (n + 2)}{2 \times \mathrm{mol \times K}}
(\#eq:molar-heat-capacities-by-constant-pressure)
\end{align}
```
[@haynes_2014_crc, pp. 1-7, A-72]

Edaspidi on võib mõista $\gamma$'t kui erisoojuste suhet, kuid ideaalse gaasi puhul saab arvutada vabadusastmete järgi. Erisoojuste suhte järgi on see:

```{=tex}
\begin{align}
\gamma := \frac{C_p}{C_V} = \frac{\frac{8.3144621(75) \times \mathrm{J} \times (n + 2)}{2 \times \mathrm{mol \times K}}}{\frac{8.3144621(75) \times \mathrm{J} \times (n + 2)}{2 \times \mathrm{mol \times K}} - \frac{8.3144621(75) \times \mathrm{J}}{\mathrm{mol \times K}}} = \frac{\frac{8.3144621(75) \times \mathrm{J} \times (n + 2)}{2 \times \mathrm{mol \times K}}}{\frac{8.3144621(75) \times \mathrm{J} \times (n + 2) - 2 \times 8.3144621(75) \times \mathrm{J}}{2 \times \mathrm{mol \times K}}} = \frac{n + 2}{n}(\#eq:ratio-of-molar-heat-capacities)
\end{align}
```
...
Milline on lõpptemperatuur adiabaatilises protsessis? Selle arvutamiseks tuleb omavahel kõrvutada töö valemid jooniselt \@ref(fig:iso-processes) ja valemist \@ref(eq:work-by-change-of-volume-and-pressure):
...
```{=tex}
\begin{align}
\overrightarrow{p} \times V = \frac{n \times `r R` \times \mathrm{J} \times T}{\mathrm{K \times mol}} 
(\#eq:ideal-gas)
\end{align}
```

Asendan ideaalse gaasi olekuvõrrandist \@ref(eq:ideal-gas) adiabaatilise töö võrdlusvõrrandisse \@ref(comparing-adiabatic-work-equations) rõhu:

```{=tex}
\begin{align}
-n \times C_V \times d(T) &=  \frac{n \times `r R` \times \mathrm{J} \times T}{\mathrm{K \times mol} \times V}\times d(V)\\
-\frac{\mathrm{K \times mol} \times n \times C_V \times d(T)}{n \times `r R` \times \mathrm{J} \times T} &=  \frac{d(V)}{V}\\
-\frac{\mathrm{K \times mol} \times C_V \times d(T)}{`r R` \times \mathrm{J} \times T} &=  \frac{d(V)}{V}\\
\int_{T_0}^T{-\frac{\mathrm{K \times mol} \times C_V \times d(T)}{`r R` \times \mathrm{J} \times T}} &= \int_{V_0}^V{ \frac{d(V)}{V}}\\
-\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} } \times \int_{T_0}^T{\frac{d(T)}{T}} &= \int_{V_0}^V{ \frac{d(V)}{V}}\\
-\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} } \times (ln(T) - ln(T_0)) &= ln(V) - ln(V_0)\\
\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} } \times (ln(T_0) - ln(T)) &= ln(V) - ln(V_0)\\
\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} } \times ln(\frac{T_0}{T}) &= ln(\frac{V}{V_0})\\
...
```{=tex}
\begin{align}
(\frac{T_0}{T})^\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} } &= \frac{V}{V_0}\\
...
```{=tex}
\begin{align}
ln(\frac{T_0}{T}) &= \frac{ln(\frac{V}{V_0})}{\frac{\mathrm{K \times mol} \times C_V}{`r R` \times \mathrm{J} }}\\
\frac{T_0}{T} &= (\frac{V}{V_0})^\frac{`r R` \times \mathrm{J}}{\mathrm{K \times mol} \times C_V}
...
```{=tex}
\begin{align}
W :&= \frac{V_0 \times C_V \times (T_0 - \frac{T_0}{(\frac{V}{V_0})^\frac{`r R` \times \mathrm{J}}{\mathrm{K} \times \mathrm{mol} \times C_V}}) \times \mathrm{K} \times \mathrm{mol} \times p}{`r R` \times \mathrm{J} \times T_0} \\
&= \frac{V_0 \times C_V \times (1 - \frac{1}{(\frac{V}{V_0})^\frac{`r R` \times \mathrm{J}}{\mathrm{K} \times \mathrm{mol} \times C_V}}) \times \mathrm{K} \times \mathrm{mol} \times p}{`r R` \times \mathrm{J}} \\
&= \frac{V_0 \times C_V \times (1 - (\frac{V_0}{V})^\frac{`r R` \times \mathrm{J}}{\mathrm{K} \times mol \times C_V}) \times \mathrm{K} \times \mathrm{mol} \times p}{`r R` \times \mathrm{J}} \\
...
$${E}_{k} = \frac{m \cdot \vec{v ^ 2}}{2}$$

$${T(\text{ideaalne gaas})} = \frac{2}{3} \cdot \frac{\overline{{E_k(\text{molekulid})}}}{1.38 \cdot 10^{-23} \cdot \mathrm{\frac{J}{K}}}$$
...
```{=tex}
\begin{align}
\eta := \frac{T_{h} - T_{c}}{T_{h}} = 1 - \frac{T_c}{T_h}
(\#eq:carnot-efficiency)
\end{align}
```

$$\eta_{ideaalne} = \frac{T_1 - T_2}{T_1} = \frac{T_{soe} - T_{külm}}{T_{soe}}$$

$$\eta_{reaalne} := \frac{Q_1 - Q_2}{Q_1} = \frac{Q_{soe} - Q_{külm}}{Q_{soe}}$$

$$\eta_{ideaalne} \ge \eta_{reaalne}$$
...
```{r label = "buckets", fig.cap = "Kaks ämbrit pealemärgitud temperatuuriga", echo = FALSE}
include_svg("heat/buckets.svg")
```
...
```{r label = "thermostate", fig.cap = "Termostaat: 1: regulaator, 3: messing, 4: raud, 2, 5: elektrijuhtmed", echo = FALSE}
include_svg("heat/thermostate.svg")
```
...
```{r road-out-of-concrete-calculations}
difference_between_enlengthening_and_distance <- l * alpha_l * change_of_temperature - d
difference_between_enlengthening_and_distance
...
$$V_0 := 150 \times \mathrm{l} = 1.5 \times 10^{-1} \times \mathrm{m^3}$$

```{r co2-expanding-isobaric-adiabatic-work-start}
V_0 <- 150e-3
T_0 <- celsius_to_kelvin(30)
```


$$t := 30\mathrm{°C} \leadsto T_0 := `r T_0` \times \mathrm{K}$$

$$V := 300 \times \mathrm{l} = 3 \times 10^{-1} \times \mathrm{m^3}$$

```{r co2-expanding-isobaric-adiabatic-work-v}
V <- 300e-3
```

$$C_V := 0.6593 \times \mathrm{\frac{kJ}{kg \times K}} = 6.593 \times 10^{2} \times \mathrm{\frac{J}{kg \times K}}$$ [@haynes_2014_crc, p. 6-36]
...
$$n := 6$$
```{r co2-expanding-isobaric-adiabatic-work-n}
n <- 6
```
...
Adiabaatilise töö arvutamiseks kasutan valemit \@ref(eq:adiabatic-work-by-volumes-molar-heat-capacity-pressure), kui tahan aluseks võtta erisoojuse. Kuna viimast ei ole ülesande püstituses esitatud, siis võib kasutada ka adiabaatilise töö integreerimise valemit \@ref(eq:adiabatic-work). Sellest on puudu sihtrõhk, mille saab arvutada konstantse rõhu ja ruumala korrutise \@ref(eq:constant-partial-work) põhjal:

```{=tex}
\begin{align}
W_i :&= \frac{\frac{p_0 \cdot V_0^\gamma \cdot V}{V^\gamma} - p_0 \cdot V_0}{1 - \gamma} = \frac{p_0 \cdot (V_0^\gamma \cdot V - V_0 \cdot V^\gamma)}{V^\gamma \cdot (1 - \gamma)} = \frac{p_0 \cdot (V_0^\frac{n + 2}{n} \cdot V - V_0 \cdot V^\frac{n + 2}{n})}{V^\frac{n + 2}{n} \cdot (1 - \frac{n + 2}{n})}\\
&= \frac{p_0 \cdot (V_0 \cdot V^\frac{n + 2}{n} - V_0^\frac{n + 2}{n} \cdot V)}{V^\frac{n + 2}{n} \cdot (\frac{2}{n})} = \frac{n \cdot p_0 \cdot (V_0 \cdot V^\frac{n + 2}{n} - V_0^\frac{n + 2}{n} \cdot V)}{V^\frac{n + 2}{n} \cdot 2}
\end{align}
```
...
W_i <- n * p * (V_0 * V^((n + 2) / n) - V_0^((n + 2) / n) * V) / (V^((n + 2) / n) * 2)
```

#### Lahend

$`r W_p` \times \mathrm{J}$ on gaasi poolt tehtud töö, kui $`r V_0` \times \mathrm{m^3}$ süsihappegaasi, mille algtemperatuur on $`r T_0` \times \mathrm{K}$, paisub isobaariliselt $`r V` \times \mathrm{m^3}$-ni.

$`r signif(W_a, digits = 7)` \times \mathrm{J}$ oleks gaasi töö juhul, kui süsihappegaas paisuks adiabaatiliselt. Kui arvestada süsihappegaasi asemel ideaalset gaasi, siis oleks gaasi adiabaatiline töö $`r signif(W_i, digits = 2)` \times \mathrm{J}$.

### Al-kalorimeeter

#### Ülesande püstitus

Alumiiniumist kalorimeetrisse massiga 80 g oli valatud vesi massiga 200 g temperatuuril 15°C. Sellesse kalorimeetrisse lasti sisse vasksilinder massiga 120g ning temperatuuril 80°C. Määrata süsteemi lõpptemperatuur. Vee erisoojus on $4190 \times \frac{J}{kg \cdot K}$, vase erisoojus on $380 \times \frac{J}{kg \times K}$, alumiiniumi erisoojus on $880 \times \frac{J}{kg \times K}$. (6p)

#### Sisendid

$$m(\mathrm{Al}) := 80 \times g = 8 \times 10^{-2} \times kg$$

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-m-al}
m_Al <- 80e-3
```


$$m(\mathrm{H_2O}) := 200 \times g = 2 \times 10^{-1} \times kg$$

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-m-h2o}
m_H2O <- 200e-3
```

$$t(\mathrm{Al, H_2O}) := 15°C \leadsto T(\mathrm{Al, H_2O}) := 2.8815 \times 10^2 K$$

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-t-al-and-h2o}
T_Al_and_H2O <- celsius_to_kelvin(15)
```

$$m(\mathrm{Cu}) := 120 \times g = 1.2 \times 10^{-1} \times kg$$

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-m-cu}
m_Cu <- 120e-3
```

$$t(\mathrm{Cu}) := 80\mathrm{°C} \leadsto T(\mathrm{Cu}) := 3.5315 \times 10^2 \times \mathrm{K}$$

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-t-cu}
T_Cu <- celsius_to_kelvin(80)
```

$$c_p(\mathrm{Al}, 300 \times \mathrm{K}) := 0.895 \times \frac{\mathrm{J}}{\mathrm{g} \times \mathrm{K}} = 8.95 \times 10^2 \times \frac{\mathrm{J}}{\mathrm{kg} \times \mathrm{K}}$$ [@haynes_2014_crc, p. 12-217]

```{r putting-copper-cylinder-into-calorimeter-of-aluminum-cp}
cp_Al <- .895e3
```

$$c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}}) := 4.1841 \times \frac{\mathrm{kJ}}{\mathrm{kg} \times \mathrm{K}} = 4.1841 \times 10^{3} \frac{\mathrm{J}}{\mathrm{kg} \times \mathrm{K}}$$ [@haynes_2014_crc, p. 6-1]

```{r cp_H2O_293k}
cp_H2O_at_293K <- 4.1841e3
```
...
Lähtun soojusliku tasakaalu valemist \@ref(eq:thermal-equilibrium):

```{=tex}
\begin{align}
Q(\mathrm{Al}) &+ Q(\mathrm{H_2O}) &+ Q(\mathrm{Cu}) &= 0\\
m(\mathrm{Al}) &\times c_p(\mathrm{Al}, 300 \times K) &\times (T - T(\mathrm{Al, H_2O}))\\
+ m(\mathrm{H_2O}) &\times c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}}) &\times (T - T(\mathrm{Al, H_2O}))\\
+ m(\mathrm{Cu}) &\times c_p(\mathrm{Cu}, 300 \times K) &\times (T - T(Cu)) &= 0\\
+m(\mathrm{Al}) &\times c_p(\mathrm{Al}, 300 \times K) &\times T\\
- m(\mathrm{Al}) &\times c_p(\mathrm{Al}, 300 \times K) &\times T(\mathrm{Al, H_2O})\\
+ m(\mathrm{H_2O}) &\times c_p(\mathrm{H_2O}, `r celsius_to_kelvin(20)` \times \mathrm{K}) &\times T\\
- m(\mathrm{H_2O}) &\times c_p(\mathrm{H_2O}, `r celsius_to_kelvin(20)` \times \mathrm{K}) &\times T(\mathrm{Al, H_2O})\\
+ m(\mathrm{Cu}) &\times c_p(\mathrm{Cu}, 300 \times \mathrm{K}) &\times T\\
- m(\mathrm{Cu}) &\times c_p(\mathrm{Cu}, 300 \times \mathrm{K}) &\times T(\mathrm{Cu}) &= 0\\
(\#eq:balanced-heat-in-al-calorimeter)
\end{align}
```

Võrrandis \@ref(eq:balanced-heat-in-al-calorimeter) toon lõpliku temperatuuri *T* sulgude ette:

```{=tex}
\begin{align}
&+T \times (\\
&&+m(\mathrm{Al}) &\times &c_p(\mathrm{Al}, 300 \times \mathrm{K})\\
&&+ m(\mathrm{H_2O}) &\times &c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}})\\
&&+ m(\mathrm{Cu}) &\times &c_p(\mathrm{Cu}, 300 \times \mathrm{K})\\
&)\\
&= T(\mathrm{Al, H_2O}) \times (\\
&&+m(\mathrm{Al}) &\times &c_p(\mathrm{Al}, 300 \times \mathrm{K})\\
&& + m(\mathrm{H_2O}) &\times &c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}})\\
&)\\
&+ T(Cu) &+ m(\mathrm{Cu}) &\times &c_p(\mathrm{Cu}, 300 \times K)\\
(\#eq:derivation-of-final-temperature-in-al-calorimeter)
\end{align}
```

Võrrandi \@ref(eq:derivation-of-final-temperature-in-al-calorimeter) teisendamisest saan nüüd avaldada lõpliku temperatuuri *T*:

```{=tex}
\begin{align}
T := (T(\mathrm{Al, H_2O}) \times (m(\mathrm{Al}) \times c_p(\mathrm{Al}, 300 \times K) + m(\mathrm{H_2O}) \times c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}}))\\
+ m(\mathrm{Cu}) \times c_p(\mathrm{Cu}, 300 \times K) \times T(Cu))\\
/ m(\mathrm{Al}) \times c_p(\mathrm{Al}, 300 \times K) + m(\mathrm{H_2O}) \times c_p(\mathrm{H_2O, `r celsius_to_kelvin(20)` \times \mathrm{K}}) + m(\mathrm{Cu}) \times c_p(\mathrm{Cu}, 300 \times K)
\end{align}
```

#### Arvutuskäik

```{r putting-cu-into-al-calorimeter-calculation}
T <- (T_Al_and_H2O * (m_Al * cp_Al + m_H2O * cp_H2O_at_293K) + m_Cu * cp_Cu * T_Cu) / (m_Al * cp_Al + m_H2O * cp_H2O_at_293K + m_Cu * cp_Cu)
```

#### Lahend

$`r signif(T, digits = 5)` \times \mathrm{K}$ on süsteemi lõpptemperatuur.

### Carnot' jõumasin

#### Ülesande püstitus

Carnot’ jõumasina, mille madalama temperatuuriga mahuti temperatuur on 17 oC, kasutegur on 50%. Kui
palju peab muutma (ja kas langetama või tõstma) madalama temperatuuriga mahuti temperatuuri, et saada
kasuteguriks 70%? Kui suur on 30s jooksul madalama temperatuuriga mahutile antud soojushulk, kui
jõumasina võimsus on 1200 W? (5p)

#### Sisendid

$$T_c(\text{initial}) := 17\mathrm{°C} = `r celsius_to_kelvin(17)` \mathrm{K}$$
```{r carnot-power-machine-t-c}
T_c_0 <- celsius_to_kelvin(17)
```

$$\eta_0 := 50\% = 5 \cdot 10^{-1}$$
```{r label = "carnot-power-machine-eta-0"}
eta_0 <- 50 / 100
```

$$\eta := 70\% = 7 \cdot 10^{-1}$$
```{r label = "carnot-power-machine-eta"}
eta <- 70 / 100
```

$$t := 30 \cdot \mathrm{s} = 3 \cdot 10 \cdot \mathrm{s}$$

```{r label = "carnot-power-machine-t"}
t <- 30
```

$$P := 1200 \times \mathrm{W} = 1.2 \times 10^3 \times \mathrm{W}$$

```{r label = "carnot-power-machine-p"}
P <- 1200
```
...
Temperatuuri arvutamiseks saab kasutada ideaalse kasuteguri valemit \@ref(eq:carnot-efficiency):

$$\Delta(T_c) := T_c(\text{final}) - T_c(\text{initial}) = T_h \cdot (1 - \eta) - T_c(\text{initial}) = \frac{T_c(\text{initial}) \cdot (1 - \eta)}{1 - \eta_0} - T_c(\text{initial})$$

Soojushulga arvutamiseks lähtun võimsuse valemist \@ref(eq:power), mille kaudu saan lasta arvutada töö, soojusmasina töö valemist \@ref(eq:work-of-heat-engine) ja soojuste suhte valemist \@ref(eq:ratio-of-heat):

```{=tex}
\begin{align}
Q_c :&= Q_h - W = \frac{Q_c \cdot T_h}{T_c(\text{initial})} - P \cdot t = \frac{Q_c \cdot T_c(\text{initial})}{T_c(\text{initial}) \cdot (1 - \eta_0)} - P \cdot t = \frac{Q_c}{(1 - \eta_0)} - P \cdot t\\
P \cdot t &= \frac{Q_c}{(1 - \eta_0)} - Q_c\\
P \cdot t &= \frac{Q_c - Q_c + Q_c \cdot \eta_0}{(1 - \eta_0)} - Q_c\\
P \cdot t &= \frac{Q_c \cdot \eta_0}{(1 - \eta_0)}\\
Q_c := \frac{P \cdot t \cdot (1 - \eta_0)}{\eta_0}
\end{align}
```

#### Arvutamine

```{r label = "carnot-power-machine-calculations"}
delta_t_c <- (T_c_0 * (1 - eta)) / (1 - eta_0) - T_c_0
...
q_c <- P * t * (1 - eta_0) / eta_0
```

#### Lahend

$`r abs(delta_t_c)` \cdot \mathrm{K}$ võrra peab `r change` madalama temperatuuriga mahuti temperatuuri, et saada kasuteguriks 70%. $`r q_c` \cdot \mathrm{J}$ on $30 \cdot \mathrm{s}$ kestel madalama temperatuuriga mahutile antud soojushulk, kui jõumasina võimsus on $1200 \cdot \mathrm{W}$.

### Mootori kasutegur

#### Ülesande püstitus

Milline on sõiduauto mootori kasutegur, kui sõiduk kulutab tunnis $5 \cdot \mathrm{l}$ diislikütust ning auto keskmine võimsus sel ajal on $20 \cdot \mathrm{kW}$? Kütuse tihedus on $700 \cdot \mathrm{\frac{kg}{m^3}}$ ning kütteväärtus $42 \cdot \mathrm{\frac{MJ}{kg}}$. Eeldades, et auto jahutusvedeliku temperatuur on $\mathrm{90°C}$ ning mootoris oleva gaasisegu tipptemperatuur on 1500°C, leida antud mootori kasutegur, kui see töötaks Carnot' tsükli alusel! (7p)

#### Sisendid

$$t := 1 \times \mathrm{h \times \frac{3.6 \times 10^3 \times s}{1 \times h}} = \mathrm{3.6 \times 10^3 \times s}$$
```{r label = "efficiency-factor-of-engine-of-car-t"}
convert_from_min_to_s <- function(min) {
  return(min * 60)
}

convert_from_h_to_s <- function(h) {
  return(convert_from_min_to_s(h * 60))
}

t <- convert_from_h_to_s(1)
```

$$V := \mathrm{5 \cdot l \cdot \frac{1 \cdot m^3}{10^3 \cdot l} = 5 \cdot 10^{-3} \cdot m^3}$$
```{r label = "efficiency-factor-of-engine-of-car-v"}
V <- 5e-3
```

$$\overline{P} := \mathrm{20 \cdot kW = 2 \cdot 10^4 \cdot W}$$
```{r label = "efficiency-factor-of-engine-of-car-p"}
P <- 20e3
```

$$\rho := \mathrm{700 \cdot \frac{kg}{m^3} = 7 \cdot 10^2 \cdot \frac{kg}{m^3}}$$
```{r label = "efficiency-factor-of-engine-of-car-rho"}
rho <- 700
```

$$\frac{Q}{m} := \mathrm{42 \times \frac{MJ}{kg} \times \frac{10^6 \times J}{1 \times MJ} = 4.2 \times 10^7 \times \frac{J}{kg}}$$

```{r label = "efficiency-factor-of-engine-of-car-q-per-m"}
Q_per_m <- 42e6
```

$$T_c := \mathrm{90°C = `r celsius_to_kelvin(90)` \times K}$$

```{r label = "efficiency-factor-of-engine-of-car-tc"}
T_c <- celsius_to_kelvin(90)
```

$$T_h := \mathrm{1500°C = `celsius_to_kelvin(1500)` \times K}$$
```{r label = "efficiency-factor-of-engine-of-car-th"}
T_h <- celsius_to_kelvin(1500)
```

#### Väljundid

$$\frac{W}{Q_h}(\mathrm{real}) := \ldots$$
$$\frac{W}{Q_h}(\mathrm{Carnot}) := \ldots$$

#### Reeglistik

Töö saan lasta arvutada võimsuse valemi \@ref(eq:work) järgi.

Sisendsoojushulga saan lasta arvutada kütteväärtuse ja massi järgi, kusjuures massi saan lasta arvutada tiheduse valemi \@ref(eq:density) abil.

Koondvalem tegeliku olukorra jaoks:

$$\frac{Q_h}{W}(\mathrm{real}) := \frac{\frac{Q}{m} \cdot \rho \cdot V}{P \cdot t}$$

Carnot' tsükli jaoks kasutan valemit \@ref(eq:coefficient-of-performance-in-carnot-heat-engine).

#### Arvutamine
```{r label = "efficiency-factor-of-engine-of-car-calculation"}
Q_h_per_W_real <- (Q_per_m * rho * V) / (P * t)
Q_h_per_W_carnot <- T_h / (T_h -  T_c)
```

#### Lahend

`r signif(Q_h_per_W_real, digits = 2)` on sõiduauto mootori kasutegur, kui sõiduk kulutab tunnis $5 \cdot \mathrm{l}$ diislikütust ning auto keskmine võimsus sel ajal on $20 \cdot \mathrm{kW}$.

`r signif(Q_h_per_W_carnot, digits = 2)` on antud mootori kasutegur, kui see töötaks Carnot' tsükli alusel eeldades, et auto jahutusvedeliku temperatuur on 90°C ning mootoris oleva gaasisegu tipptemperatuur 1500°C.
...
$$\bar{\epsilon}_{kin} = \frac{i}{2} \cdot k \cdot T$$
...
$$m_{oil} \cdot c_{oil} \cdot (T - T_{oil}) === -m_{CU} \cdot c_{CU} \cdot (T - T_{CU}) - C \cdot (T - T_{oil})$$
...
Madalal tiigil on moodustunud jää ja saabunud on statsionaarne olukord, kus
õhutemperatuur jää kohal on -5oC ja veetemperatuur tiigi põhjas on +4oC. Kui tiigi
sügavus (jää ja vee kogupaksus) on 1,4 m, siis kui paks on jää? Jää
soojusjuhtivustegur on 2,1 W/(m·K) ning vee soojusjuhtivustegur on 0,592
W/(m·K).
...
$$\kappa_1 = 2.1 \times \frac{W}{m \times K}$$

$$\kappa_2 = 5.92 \times 10^{-1} \times \frac{W}{m \times K}$$
...
Soojuslik tasakaal saabub siis, kui läbi veekihi läheb samasugune soojusvoog kui läbi jääkihi. Peab arvestama, et otse jääkihi all on temperatuur 0°C.

$$q = \frac{\Delta(Q)}{\Delta(t)} = \frac{k \times A \times \Delta(t)}{l}$$

$$q_1 := \frac{k_1 \times A \times (T_2 - T_1)}{l_1}$$

$$q_2 := \frac{k_2 \times A \times (T_3 - T_2)}{l_2}$$
$$q_2 := \frac{k_2 \times A \times (T_3 - T_2)}{l - l_1}$$

$$q_1 = q_2$$

$$\frac{k_1 \times A \times (T_2 - T_1)}{l_1} = \frac{k_2 \times A \times (T_3 - T_2)}{l - l_1}$$

$$k_1 \times (T_2 - T_1) \times (l - l_1) = k_2 \times l_1 \times (T_3 - T_2)$$

$$k_1 \times (T_2 - T_1) \times l - k_1 \times (T_2 - T_1) \times l_1 = k_2 \times l_1 \times (T_3 - T_2)$$

$$k_2 \times l_1 \times (T_3 - T_2) +  k_1 \times (T_2 - T_1) \times l_1 = k_1 \times (T_2 - T_1) \times l$$

$$l_1 \times (k_2 \times (T_3 - T_2) + k_1 \times (T_2 - T_1)) = k_1 \times (T_2 - T_1) \times l$$

$$l_1 := \frac{k_1 \times (T_2 - T_1) \times l}{k_2 \times (T_3 - T_2) + k_1 \times (T_2 - T_1)}$$

```{r}
l_1 <- (2.1 * (273 - 268) * 1.4) / (5.92e-1 * (277 - 273) + 2.1 * (273 - 268))
l_1
```
...
Carnot' soojusmasina jahutaja temperatuur on 0oC. Leida soojendaja
temperatuur, kui masin annab 1/3 soojendajalt saadud soojuselt jahutajale.

#### Tegu

$$T_{külm} := 0°C = 273 \times K$$

$$Q_{külm} := \frac{1}{3} \times Q_{soe}$$

$$T_{soe} := \ldots$$

$$\eta_{ideaalne} = \frac{T_{soe} - T_{külm}}{T_{soe}}$$

$$\eta = \frac{Q_{soe} - Q_{külm}}{Q_{soe}} = 1 - \frac{Q_{külm}}{Q_{soe}}$$

$$\eta = \eta_{ideaalne}$$

$$1 - \frac{T_{külm}}{T_{soe}} = 1 - \frac{Q_{külm}}{Q_{soe}}$$

$$T_{soe} := \frac{T_{külm} \times Q_{soe}}{Q_{külm}}$$
...
Carnot' ringprotsessil teeb gaas iga soojendajalt saadud 4187J arvel 598J tööd.
Kui suur on protsessi kasutegur? Mitu korda on soojendaja absoluutne
tempeteratuur kõrgem jahutaja absoluutsest temperatuurist?

#### Tegu

$$Q := 4.187 \times 10^3 \times J$$

$$W := 5.98 \times 10^2 \times J$$

$$\Delta(Q) = \Delta(U) + W$$

$$\eta = \frac{Q_{soe} - Q_{külm}}{Q_{soe}}$$
...
$$\eta_{ideaalne} := \frac{T_{soe} - T_{külm}}{T_{soe}} = 1 - \frac{T_{külm}}{T_{soe}}$$

$$\eta_{ideaalne} = \eta_{reaalne}$$
